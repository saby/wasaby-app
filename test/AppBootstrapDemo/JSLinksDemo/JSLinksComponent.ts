    ...
    _beforeMount(
        ...
        if (!constants.isBrowserPlatform) {
            ...
            JSLinksAPI.createTag('script', {
                src: ModulesLoader.getModuleUrl(
                    'AppBootstrapDemo/JSLinksDemo/JSLinksResource.js'
                ),
            });
            this.jslinksData += new TagMarkup(
                JSLinksAPI.getData().map(fromJML),
        ...
        // @ts-ignore
        this.isBuildnumberString =
            typeof window.buildnumber === 'string' ? 'success' : 'fail';
    }
