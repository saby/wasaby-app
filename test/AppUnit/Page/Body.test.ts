...
describe('Application/_Page/Body', () => {
    it('Инициализация стартовых значений теста', () => {
        ...
        assert.isTrue(typeof classes === 'string', 'Стартовый набор классов не является строкой.');
    });
    ...
    it('Добавление класса', () => {
        ...
        assert.equal(API.getClassString(), classes, 'Добавление одного класса сработало неверно');

        API.addClass.apply(API, classesToAdd);
        classes = [classes, classesToAdd.join(' ')].join(' ').trim();
        assert.equal(API.getClassString(), classes, 'Добавление нескольких классов сработало неверно');
    });
    ...
    it('Удаление класса', () => {
        ...
        assert.equal(API.getClassString(), [
            classes,
            classesToCheck.join(' ')
        ].join(' ').trim(), 'Удаление одного класса сработало неверно');

        API.removeClass.apply(API, classesToCheck);
        assert.equal(API.getClassString(), classes, 'Удаление нескольких классов сработало неверно');
    });
    ...
    it('Проверка переключателя классов', () => {
        ...
        assert.equal(API.getClassString(), [classes, classToCheck].join(' ').trim(), 'Не нашлось искомого класса после добавления через toggle');

        API.toggleClass(classToCheck, true);
        assert.equal(API.getClassString(), [classes, classToCheck].join(' ').trim(), 'Появился дубль класса после toggle');

        API.toggleClass(classToCheck);
        assert.equal(API.getClassString(), classes, 'Нашелся искомый класс после удаления через toggle');
