...
/**
 * Название шаблона десериализации даты,
 * который нужно исключить при десериализации данных
 */
const DATE_PATTERN_NAME = 'DATE';
...
export class Serializer {
    ...
    static setIgnorePatternName(_: string): void {}
    static deleteIgnorePatternName(_: string): void {}
}
...
const logger: IConsole = {
    ...
    setLogLevel(_logLevel: number): void {},
...
/**
 * Класс предназначенный для сохранения состояний компонентов при построении на СП
 * и последующего восстановления этого состояния на клиенте.
 *
 * Напрямую использовать этот класс нельзя.
 *
 * @public
 * @see Application/Env#getStateReceiverData
 */
export class StateReceiver implements IStateReceiver {
    private _receivedStateObjectsArray: Record<string, IReceivedStateData> = {};
    private _receivedStatePromisesArray: Record<string, Promise<IReceivedStateData | void>> = {};
    private _deserialized: any = {};
    private _serialized: ISerializedType;
    private __serializer: unknown;
    ...
    serialize(): ISerializedType {
        ...
        const allAdditionalDeps: Record<string, boolean> = {};
        ...
        Object.keys(allRecStates).forEach((key) => {
            ...
            try {
                ...
                this._getLogger().error(getSerializeError(meta?.moduleName, key, e as Error), e);
                delete serializedMap[key];
            }
        });

        // Подменяем Date.toJSON, чтобы при сериализации объект Date превращался в спец. структуру.
        // Тем самым десериализуем в Date только эти спец. структуры.
        // Нужно, чтобы строки с датой ошибочно не десериализовать.
        const origToJSON = Date.prototype.toJSON;
        // @ts-ignore
        Date.prototype.toJSON = function () {
            return { $serialized$: TYPE_DATE, state: origToJSON.call(this) };
        };

        /** сериализация словаря */
        let serializedState = JSON.stringify(serializedMap, slr.serializeStrict);

        // Вернем обратно оригинальный Date.toJSON
    ...
    deserialize(str: string | undefined): void {
        ...
        const slr = this.__getSerializer();
        this._constructorSerializer.setIgnorePatternName(DATE_PATTERN_NAME);
        try {
            this._deserialized = JSON.parse(str, slr.deserialize);
        } catch (error) {
            this._getLogger().error(`Ошибка десериализации ${str}`, null, error);
        }
        this._constructorSerializer.deleteIgnorePatternName(DATE_PATTERN_NAME);
    }
    ...
    register(
        ...
        if (data instanceof Promise) {
            return this._registerPromise(meta, data as Promise<ISerializableState>);
        }

        this._registerInst(meta, data as ISerializableState, guess);
    }

    _registerPromise(meta: string | IStateReceiverMeta, data: Promise<ISerializableState>): void {
    ...
    private _guessResult(key: string, guess: boolean = false): Record<string, any> | void {
        ...
        this.preparedKeys =
            this.preparedKeys ||
            this.deserializedKeys.map((keyFromDeserialized) => {
...
function getSerializeError(moduleName: string, field: string, error: Error): string {
