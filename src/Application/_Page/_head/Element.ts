...
export default class Element extends BaseElement {
    constructor(
        ...
        const data: IElementRestoredData =
            Element._restoreElement(hydrateElement);
        super(
            data.name || name,
            data.attrs || attrs,
    ...
    changeTag(attrsChange: IPageTagAttrs): void {
        ...
        const allItems = Object.getOwnPropertyNames({
            ...attrs,
            ...attrsChange,
        });
    ...
    protected _startEvents() {
        ...
        if (this._eventHandlers?.error) {
            this._element.addEventListener(
                'error',
                this._eventHandlers.error as EventListener
            );
        }
    }
     ...
     * @private
     */
    protected static _restoreElement(
        element?: HTMLElement
    ): IElementRestoredData {
