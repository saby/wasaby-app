...
export class JSLinksAspect implements IPageTagAPIAspect {
    ...
    isCrossOrigin(src: string): boolean {
        return (
            (src.startsWith('//') || src.startsWith('http')) && src.includes(`//${location.host}`)
        );
    }
    ...
    createNoScript(_url: string): void {
        return;
    }
    ...
    createTag(
        ...
        _elements: { [propName: string]: IPageTagElement }
    ): IPageTagElement | void {
        const errorPrefix = 'Application/_Page/_jsLinks/Aspect';

        if (typeof window !== 'undefined') {
            logger.warn(`${errorPrefix} Вызывать метод createTag на клиенте запрещено.`);
            return;
        }
        if (name !== 'script') {
            throw new Error(
    ...
    createMergeTag(_name: string, _attrs: IPageTagAttrs, _content: string): void {
        logger.error(new Error('Для скриптов в JSLinks API этот метод не реализован'));
    }

    getComments(_wrap?: boolean): string[] {
        return [];
    }
