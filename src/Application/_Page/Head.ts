...
export class Head extends PageTagAPI {
    ...
    private _collectTags(): void {
        ...
        Array.from(document.head.children).forEach((item: Element) => {
            // Нет смысла собирать noscript тег. В живой странице ты его не применишь.
            if (item.tagName === 'NOSCRIPT') {
                return;
            }
            // @ts-ignore create(null - по типу только строка, может тут следует писать create(''
            const element = create(null, null, null, null, item);
            this._elements[element.getAttrs()[SERVER_ID_FIELD] || this._aspect.generateGuid()] =
                element;
        });
    }
