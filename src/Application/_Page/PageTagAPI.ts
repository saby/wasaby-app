...
export class PageTagAPI implements IPageTagAPI {
    ...
    createTag(name: 'title', attrs: {} | { class: string }, content: string): IPageTagId;
    createTag(
        name: 'script',
        attrs:
            | {}
    ...
    // @ts-ignore content не может быть null по типу, но есть проерки на null
    createTag(
        name: 'link',
        attrs: IPageTagAttrs,
        content: null,
    ...
    createTag(
        ...
        this._elements[attrs[SERVER_ID_FIELD]] = this._aspect.createTag(
            ...
            attrs,
            content as string,
            eventHandlers as IPageTagEventHandlers,
            this._elements
        ) as IPageTagElement;
